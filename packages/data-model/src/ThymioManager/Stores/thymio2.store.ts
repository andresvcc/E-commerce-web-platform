...
// store class for Thymio2 of type Thymio
@Store({ key: 'Thymio Store', predicate: ['thymio2'] })
export class Thymio2 implements Thymio {
  ...
  connectToMqttBroker = (host: string, token: string) => {
    // console.log('connectToMqttBroker on thymio 2', host, token);
  };

  // connect the thymio to the websocket server
  connectToWebsocket = (host: string, token: string) => {
    // console.log('connectToWebsocket on thymio 2', host, token);
  };
