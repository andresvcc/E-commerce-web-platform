...
// store class for Thymio3 of type Thymio
@Store({ key: 'Thymio Store', predicate: ['thymio3'] })
export class Thymio3 implements Thymio {
  // uuid of the thymio (unique identifier)
  uuid: string;
  type: ThymioType = 'thymio3';
  ...
  connectToMqttBroker = (host: string, token: string) => {
    console.log('connectToMqttBroker on thymio 3', host, token);
  };

  // connect the thymio to the websocket server
  connectToWebsocket = (host: string, token: string) => {
    console.log('connectToWebsocket on thymio 3', host, token);
  };
