...
import { useArrow, useGlobalListener, useOpen, usePosition, useTrigger } from './hooks';
...
const Popover: ForwardRefRenderFunction<HTMLElement, PopoverProps> = (
    ...
    <AnimatePresence initial={null}>
      {isOpen && (!usePortal || containerStyle) && (
        ...
        >
          ...
          <div className="idui-popover__content" style={{ maxHeight, maxWidth, ...contentStyle }}>
            {transformedContent}
          </div>
        </motion.div>
      )}
  ...
  if (trigger === PopoverTriggerType.focus) {
    let childElement: ReactChild = (
      isFunction(children) ? children({ isOpen, open, close, toggle }) : children
    ) as ReactChild;
    ...
    <Fragment>
      {shouldCheckContentDimensions &&
        ...
        createPortal(
          ...
          >
            <div className="idui-popover__content" style={{ maxHeight, maxWidth }}>
              {transformedContent}
            </div>
          </div>,
          container,
      ...
      {usePortal ? Boolean(container) && createPortal(popoverContentAnimated, container) : popoverContentAnimated}
