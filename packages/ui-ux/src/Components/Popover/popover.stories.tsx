...
export default {
  ...
  argTypes: {
    ...
    onFocus: {
      action: 'onFocus',
      description: "onFocus event of child component, triggered if trigger === 'focus'",
    },
    withArrow: {
      control: 'boolean',
    ...
    content: {
      disable: true,
      description: "Popover content. If it's function then it provided with {close: close popover}",
    },
    offset: {
      control: 'array',
      description:
    ...
    getContainer: {
      disable: true,
      description: 'Function, that should return component inside which popover should render its content',
      table: {
    ...
    considerTriggerMotion: {
      control: 'boolean',
      description: 'Whether consider trigger position and size changes and follow it or not',
      defaultValue: false,
      table: {
        defaultValue: { summary: false },
      },
    ...
    animation: {
      control: 'object',
      description: 'framer-motion props for opening/closing content animation {initial, animate, exit}',
      defaultValue: {
    ...
    arrowOffset: {
      control: 'number',
      description: 'arrow offset (px) from popover side (second placement part) if placement consists of two sides',
      defaultValue: 10,
      table: {
    ...
    arrowPlacement: {
      ...
      description: 'arrow placement (left|center|right for top|bottom and top|center|bottom for right|left)',
      table: {
        defaultValue: {
          summary: 'second popover placement part if it consists of two sides else center',
        },
      },
    },
    maxWidth: {
    ...
    lazy: {
      control: 'boolean',
      description: 'whether wait for trigger event to initialize popover or not',
      defaultValue: true,
      table: {
        defaultValue: { summary: true },
      },
...
export const Playground: ComponentStory<typeof Popover> = (props) => {
  return (
    <Popover {...props}>
      <button>Trigger &quot;{props.trigger || 'hover'}&quot; event to open Popover</button>
    </Popover>
  );
};
...
export const DraggablePopover: ComponentStory<typeof Popover> = (props) => {
  return (
    ...
    >
      ...
      <Popover {...props} considerTriggerMotion getContainer={() => document.getElementById(dragContainerId)}>
        <motion.div
          style={{
            width: 100,
            height: 100,
  ...
  return (
    <div>
      <div style={{ position: 'relative', width: '500px' }}>
        ...
        <input type="range" min={0} max={100} onChange={handleChangeRange} value={left} style={{ width: '100%' }} />
      </div>
      <div className="note">Note: popover is visible and visibility controlled only for example</div>
      <div className="note">
        Note: triggerContainerDisplay provided because popover computes wrong value in storybook..
      </div>
    </div>
  );
};
...
export const PopoverWithLongContent: ComponentStory<typeof Popover> = (props) => {
  return (
...
export const PopoverWithFunctionChildren: ComponentStory<typeof Popover> = (props) => {
  return (
    <Popover
      {...props}
      considerTriggerMotion
...
export const PopoverWithFocusTrigger: ComponentStory<typeof Popover> = (props) => {
  return (
    <div>
      <Popover {...props} content="I was opened on focus" trigger={PopoverTriggerType.focus}>
        <div>
          <input type="text" placeholder="Focus" />
        </div>
      </Popover>
      <div className="note">Note: Input uses forwardRef for changing trigger component to input</div>
    </div>
  );
};

export const PopoverWithCustomSimpleAnimation: ComponentStory<typeof Popover> = (props) => {
  return (
    <Popover
      {...props}
      content={longContent}
...
export const PopoverWithCustomArrow: ComponentStory<typeof Popover> = (props) => {
  return (
    <Popover
      {...props}
...
export const CloseOnScroll: ComponentStory<typeof Popover> = (props) => {
  return (
    <div style={{ overflow: 'auto', width: 300, height: 300 }}>
      <div style={{ backgroundColor: 'orangered', height: 100, marginBottom: 10 }} />
      <CustomPopover {...props} content="Hi!">
        <button>{props.trigger || 'hover'} to open and scroll to check</button>
      </CustomPopover>
      <div style={{ backgroundColor: 'orangered', height: 500, marginTop: 10 }} />
    </div>
  );
};
